...
| Capture / scraping (**primaire**) | **Firecrawl** v3 (VPS IONOS) | ✅ | `http://10.10.0.1:3002` via **WireGuard** — joignable depuis l'hôte ET les conteneurs du réseau `coolify` (vérifié 2026-05-27 : `/` = 200, `/v1/scrape` OK). Aucune clé. Dev local → tunnel SSH. |
| Capture / scraping (~~fallback~~) | ~~`crawl4ai`~~ | ❌ | **Déprécié/retiré du code (2026-05-27)** : domaine public `crawl4ai.augmenter.pro` → 404 (route Traefik retirée), conteneur hors réseau `coolify`, aucun port hôte. |
...
Capture site                       → Firecrawl 10.10.0.1:3002 (WireGuard) ✅  (crawl4ai retiré)
Résumé / extraction                → litellm fast4b / groq-fast ✅  (workers Celery + shared_redis ✅)
Authority Score (SEO+GEO)          → GSC API + Firecrawl ✅ ; proxy GEO = pgvector ✅ ; Sonar = API externe (à câbler)
...
- **Réseau** : les services partagés écoutent sur `127.0.0.1` (host) **et** sont sur le réseau Docker `coolify`. Les conteneurs WeBuild devront **rejoindre le réseau `coolify`** pour `shared_postgres:5432`, `shared_redis:6379`, `bge_reranker:8001`. ⚠️ Nuance vérifiée 2026-05-27 : les conteneurs Coolify portent un **suffixe** (`litellm-b4w8…`) sans alias court `litellm` → côté app, **LiteLLM est adressé par son domaine public** (`https://litellm.augmenter.pro`), pas par nom interne. **Crawl4AI** n'est pas sur `coolify` (retiré). **Firecrawl** = `http://10.10.0.1:3002` via **WireGuard** (MASQUERADE host), joignable depuis tout conteneur du host.
